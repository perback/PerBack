...
public class BaseDao
{
	...
	protected Serializable read(StorageMethod storageMethod, Serializable defaultValue, String... keys)
	{
		...
		if (data == null)
		{
			...
			{
				case SharedPreferences: data = readSharedPrefs(path); break;
				case InternalFiles: case ExternalFiles: data = readFileSystem(storageMethod, path); break;
			}
	...
	protected void write(StorageMethod storageMethod, Serializable data, String... keys)
	{
		...
		switch (storageMethod)
		{
			case SharedPreferences: writeSharedPrefs(path, data); break;
			case InternalFiles: case ExternalFiles: writeFileSystem(storageMethod, path, data); break;
		}
		writeHeapCache(path, data);
	}
	...
	private Serializable readHeapCache(String path)
	{
		...
		return returnValues.size() > 1 ? returnValues : !returnValues.isEmpty() ? returnValues.get(0) : null;
	}
	...
	{
		ArrayList<Serializable> returnValues = new ArrayList<Serializable>();
		Map<String, ?> sharedPrefsValues = context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).getAll();
	...
	private void writeSharedPrefs(String path, Serializable data)
	{
		if (data != null)
		{
			if (data instanceof Boolean)
				context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().putBoolean(path, (Boolean)data).commit();
			else if (data instanceof Integer)
				context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().putInt(path, (Integer)data).commit();
			else if (data instanceof Long)
				context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().putLong(path, (Long)data).commit();
			else if (data instanceof Float)
				context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().putFloat(path, (Float)data).commit();
			else
				context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().putString(path, data.toString()).commit();
		}
		else
			context.getSharedPreferences(SHARED_PREFS_FILE, Context.MODE_PRIVATE).edit().remove(path).commit();
	}
	...
	{
		return new File((storageMethod == StorageMethod.InternalFiles ? context.getFilesDir() : context.getExternalFilesDir(null))
